...
checkHubcapSub :: Int -> TpAxleI -> [Int] -> [(Int, Int, Int)] -> TpReducePack -> Int
  -> RWST ([TpGoodConf], TpPosout, Int) () (TpReducePack, [Int]) IO String
checkHubcapSub i axL s xyvList rP nouts
  ...
  | otherwise           = do
      ...
      checkHubcapSub (i + 1) axL s xyvList rP nouts where
        (xi, yi, vi) = xyvList !! i
...
checkBoundSub1 i forcedch allowedch retS axL rules posoutX deg
  ...
  | otherwise       = checkBoundSub1 (i + 1) forcedch3 allowedch2 retS2 axL rules posoutX deg where
      rVi       = (rules ^. _3) !! i
...
checkBoundSub5Sub1 i x (axLowL, axUppL) rules pos deg
  ...
  | otherwise                   = checkBoundSub5Sub1 (i + 1) x (axLowL2, axUppL2) rules pos deg where
      p0      = ((rules ^. _4) !! pos) !! i
      ...
      (axLowL2, axUppL2)
        ...
        | otherwise                                        = error "checkBoundSub5Sub1 error!"
...
checkBoundSub5Sub2 i x depth s pos rules (axLowL2, axUppL2) posoutX deg
  ...
  | otherwise                 = checkBoundSub5Sub2 (i + 1) x depth s pos rules (axLowL2, axUppL2) posoutX deg where
