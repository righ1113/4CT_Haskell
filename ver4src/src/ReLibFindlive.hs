...
findlive ring bigno live ncodes angle _power extentclaim
  = findliveSub (ring, ncodes, 0, extentclaim, (False, c3, j), ed) bigno live angle forbidden2 0 where
      ed             = head angle !! 2
      c              = replicate edges 0
      j              = ed - 1
      c2             = c  & ix ed .~ 1
...
findliveSub (ring, ncodes, extent, extentclaim, (_, c, j), ed) bigno live angle forbidden cnt = do
  print angle
  ...
  case () of
    _ | exit1                   -> return (ncodes - extent,  live)
      ...
      | otherwise ->
          findliveSub (ring, ncodes, extent2, extentclaim, (False, cNext, jNext), ed) bigno live2 angle forbi2 (cnt + 1)
