...
findLiveSub (ring, nCodes, extent, extentC, (_, c, j), ed) bigNo live angle forbi cnt = do
  ...
  ((exit1, _, j2), (extent2, live2), (exit2, _, _), (exit3, _, _), (_, cNext, jNext), forbi2)
    <- findLiveSsub5 ring
...
beforePrintStatus (ring, nCodes, extent, extentC, (_, c, j), ed) = do
  ...
  print ed
  print ring-}
  flip fix (c2, j) $ \loop (c0, j0) -> case () of
    _ | 8 .&. (c0 !! j0) == 0 -> return (False, c0, j0)
      | j0 >= ed - 1        -> do{ printStatus ring nCodes extent extentC; return (True, c0, j0) }
