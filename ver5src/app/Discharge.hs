...
import CoLibCConst
    ( TpReducePack,
      ...
      maxelist,
      maxlev )
import DiLibApply                ( delSym, chkApply )
import DiLibCaseSplit            ( caseSplit1, caseSplit2 )
import DiLibDischarge            ( discharge )
...
mainLoop (nn, mm) sym@(_, symNol, _, _, _, _) nosym ax@(axLow, axUpp, axLev) tactics lineno
  ...
  | otherwise       = let nowTac = head tactics in
      case nowTac !! 1 of
        "C" -> do
          liftIO . putStrLn $ "CaseSplit  " ++ show nowTac
          (_, _, deg)              <- ask
          let n                     = read (head tactics !! 2) :: Int
              m                     = read (head tactics !! 3) :: Int
              (low2, upp2, _axLev2) = caseSplit1 (nn, mm) ax n m
              (sym2, nosym2)        = caseSplit2 (nn, mm) ax deg sym nosym lineno
        ...
        "H" -> do
          liftIO . putStrLn $ "Discharge  " ++ show nowTac
          _ <- discharge (tail (tail (head tactics))) ax
        ...
        "R" -> do
          ...
          if isNothing ret then
            ...
            mainLoop (nn, mm) sym nosym2 (axLow, axUpp, axLev - 1) (tail tactics) (lineno + 1)
        "S" -> do
          liftIO . putStrLn $ "Apply  " ++ show nowTac
          (_, _, deg) <- ask
          liftIO $ chkApply (tail (tail (head tactics))) ax sym nosym deg
